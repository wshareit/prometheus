...
func (c *LeveledCompactor) populateBlock(blocks []BlockReader, meta *BlockMeta, indexw IndexWriter, chunkw ChunkWriter) (err error) {
	...
	set := storage.NewMergeChunkSeriesSet(sets, storage.NewCompactingChunkSeriesMerger(storage.ChainingSeriesMerge))
	for set.Next() {
		select {
		case <-c.ctx.Done():
			return c.ctx.Err()
...
func (p *populateWithTombstonesChunkIterator) Next() bool {
	for p.err == nil && p.i < len(p.chks)-1 {
		...
		//
		// It is mostly for blockChunkQuerier, but also for compactor. During (cut) compaction,
